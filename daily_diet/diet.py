...
def verify_diet(input):
    result=True
    if (type(input["protein"]) != float ) or (input["protein"]<0): #g
        result = False
    elif (type(input["fat"]) != float ) or (input["fat"]<0): #g
        result = False
    elif (type(input["carbs"]) != float ) or (input["carbs"]<0): #g
        result = False   
    elif (type(input["amount"]) != float ) or (input["amount"]<0):
        result = False             
    return result     
...
def handle_delete_diet(request):
        ...
        if isinstance(connection,Connection): #如果有順利取得連線
            result = connection.delete_diet(intake_id) 
